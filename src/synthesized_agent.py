                ...
                if perspective != other_perspective:
                    ...
                    if unique_to_perspective:
                        divergences.append({
                            ...
                            'unique_focus': [t.value for t in unique_to_perspective],
                            'insights': [i.content for i in insights if i.type in unique_to_perspective]
                        })
...
class ResearchDigestionAgent(BaseΣAgent):
                ...
                for research_file in self.research_files:
                    ...
                    if file_path.exists():
                        insights = await self._process_research_file(
                            file_path,
                            ResearchPerspective[model_dir.upper().replace("-", "_").replace("4_", "4_")]
                        )
    ...
    async def _process_research_file(self, file_path: Path, perspective: ResearchPerspective) -> List[ResearchInsight]:
...
class ImplementationPlanAgent(BaseΣAgent):
    ...
    def _define_milestones(self) -> List[Dict[str, str]]:
        ...
        return [
            ...
            {"milestone": "M2", "description": "Agent implementations and orchestration"},
...
if __name__ == "__main__":
    ...
    async def main():
        request = TaskRequest(
            description="Execute full Σ-Builder workflow",
            config={"verbose": True}
        )
